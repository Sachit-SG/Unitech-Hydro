...
export function SiteHeader() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-100 bg-white shadow-sm">
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between gap-4 px-6 md:px-20">
        <Link
          href="/"
          ...
          {navItems.map(({ href, label }) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  "rounded-[4px] px-4 py-2 text-[15px] font-medium tracking-wide transition-colors",
                  isCta
                    ? "ml-2 bg-[#00EAFF] px-6 py-2.5 font-bold text-[#0B2043] hover:bg-[#00EAFF]/90"
                    : active
